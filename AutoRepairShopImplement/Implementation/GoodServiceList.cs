...
namespace AutoRepairShopImplement.Implementation
{
    public class GoodServiceList : IGood
    {
        ...
        {
            List<GoodView> result = new List<GoodView>();
            for (int i = 0; i < source.Products.Count; ++i)
            {
                // требуется дополнительно получить список компонентов для изделия и их количество
            List<GoodComponentView> productComponents = new List<GoodComponentView>();
                for (int j = 0; j < source.ProductComponents.Count; ++j)
                {
                    if (source.ProductComponents[j].ProductId == source.Products[i].Id)
                    {
                        ...
                        productComponents.Add(new GoodComponentView
                        {
